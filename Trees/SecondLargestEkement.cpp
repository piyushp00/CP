/* Given a generic tree, find and return the node with second 
largest value in given tree.
...
TreeNode<int>* secondLargestElement(TreeNode<int>* root, int n){
    if(root == NULL){
        return root;
    }
	...
	for(int i = 0; i < root->children.size(); i++){
        ...
        if(root->children[i]->data < largest){
            ans = root->children[i];
